package com.example.chenrui.favoriteitem;

import android.content.Context;
import android.graphics.Bitmap;
import android.graphics.BitmapFactory;
import android.support.v4.util.LruCache;
import android.util.Log;
import android.widget.Toast;

import com.yelp.fusion.client.models.Business;
import com.yelp.fusion.client.models.SearchResponse;

import org.json.JSONArray;
import org.json.JSONException;
import org.json.JSONObject;

import java.io.IOException;
import java.io.InputStream;
import java.net.HttpURLConnection;
import java.net.URL;
import java.util.ArrayList;
import java.util.List;

/**
 * Created by chenrui on 2018/8/5.
 */

public class DataService {

    private LruCache<String, Bitmap> bitmapCache;

    /**
     * Constructor.
     */
    private Context mContext;
    /**
     * Constructor.
     */

    /**
     * Constructor.
     */
    public DataService() {
        // Get max available VM memory, exceeding this amount will throw an
        // OutOfMemory exception.
        final int maxMemory = (int) (Runtime.getRuntime().maxMemory() / 1024);

        // Use 1/8th of the available memory for this memory cache.
        final int cacheSize = maxMemory / 4;
        Log.e("Cache size", Integer.toString(cacheSize));

        bitmapCache = new LruCache<String, Bitmap>(cacheSize) {
            @Override
            protected int sizeOf(String key, Bitmap bitmap) {
                // The cache size will be measured in kilobytes.
                return bitmap.getByteCount() / 1024;
            }
        };
    }




    public List<Restaurant> getNearbyRestaurants() {
        YelpApi yelp = new YelpApi();
        Log.d("san fran","success!!!");
        SearchResponse jsonResponse = yelp.searchForBusinessesByLocation("dinner", "San Francisco, CA", 20);

        return parseResponse(jsonResponse);
    }

    public List<Restaurant> getNearbyRestaurants(String longitude, String latitude) {
        YelpApi yelp = new YelpApi();
        SearchResponse jsonResponse = yelp.searchForBusinessesByCoordinate("dinner", longitude,latitude, 20);
        Log.d("check","Success!!!!!!");
        return parseResponse(jsonResponse);
    }

    private List<Restaurant> parseResponse(SearchResponse Response)  {
        try {
            Log.d("parse","success1");
            ArrayList<Restaurant> restaurants = new ArrayList<Restaurant>();
            ArrayList<Business> businesses = Response.getBusinesses();
            Log.d("num",Integer.toString(businesses.size()));
            for(int i=0; i<businesses.size(); i++) {
                Business business = businesses.get(i);
                if (business != null) {
                    String name = business.getName();
                    double rating = business.getRating();
                    String type = business.getCategories().get(0).getTitle();
                    String address = business.getLocation().getAddress1();
                    double lat = business.getCoordinates().getLatitude();
                    double lng = business.getCoordinates().getLongitude();
                    Bitmap thumbnail =  getBitmapFromURL(business.getImageUrl());
                    restaurants.add(new Restaurant(name, address, type, lat, lng, thumbnail,rating));
                }

            }
            /*JSONObject json = new JSONObject(jsonResponse);
            JSONArray businesses = json.getJSONArray("businesses");
            ArrayList<Restaurant> restaurants = new ArrayList<Restaurant>();
            for (int i = 0; i < businesses.length(); i++) {
                JSONObject business = businesses.getJSONObject(i);

                //Parse restaurant information
                if (business != null) {
                    String name = business.getString("name");
                    String type = ((JSONArray) business.get("categories")).
                            getJSONArray(0).get(0).toString();

                    JSONObject location = (JSONObject) business.get("location");
                    JSONObject coordinate = (JSONObject) location.get("coordinate");
                    double lat = coordinate.getDouble("latitude");
                    double lng = coordinate.getDouble("longitude");
                    String address =
                            ((JSONArray) location.get("display_address")).get(0).toString();

                    // Download the image.
                    Bitmap thumbnail =  getBitmapFromURL(business.getString("image_url"));
                    Bitmap rating = getBitmapFromURL((business.getString("rating_img_url")));

                    restaurants.add(new Restaurant(name, address, type, lat, lng, thumbnail,rating));
                }
            }*/
            return restaurants;
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    }
    public Bitmap getBitmapFromURL(String imageUrl) {
        Bitmap bitmap = null;
        if (bitmapCache != null) {
            bitmap = bitmapCache.get(imageUrl);
        }

        if(bitmap == null) {
            try {
                URL url = new URL(imageUrl);
                HttpURLConnection connection = (HttpURLConnection) url.openConnection();
                connection.setDoInput(true);
                connection.connect();
                InputStream input = connection.getInputStream();
                bitmap = BitmapFactory.decodeStream(input);
                if (bitmapCache != null) {
                    bitmapCache.put(imageUrl, bitmap);
                }

            } catch (IOException e) {
                e.printStackTrace();
                Log.e("Error: ", e.getMessage().toString());
            }
        }
        return bitmap;
    }



}


